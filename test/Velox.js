...
describe("VLX", function () {
   this.beforeEach(async function () {
      [owner, addressA, addressB, addressC, holovCore] =
   ...
   describe("Deployment", function () {
      ...
      it("should prove that owner has the MINTER_ROLE", async function () {
         expect(
            await contract.hasRole(
   ...
   describe("Transactions", function () {
      it("should allow axp_sys defined address to burn vlx using utilBurnFor", async function () {
         ...
         expect(await contract.balanceOf(addressA.address)).to.equal(
            utils.parseEther((50000 - 50).toString())
         );
      });
